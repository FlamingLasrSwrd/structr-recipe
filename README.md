...
- `docs/` — the design documents. `data-model.md` and `structr-build-sketch.md` are authoritative; `structr-cheatsheet.md` is empirically verified Structr mechanics; `CLAUDE.md` holds the hard rules this build followed and a document-authority table. Read `CLAUDE.md` first. `optimizer-design.md` is the unreviewed design of the plan-level planner; its section 12 records what was built, what was assumed, and the measured limits. The two `design-review-document*.md` files are historical: they describe the model *before* their own fixes and should not be followed.
...
- `tools/` — `snapshot_state.py` dumps the graph and the schema (including every validator's source) as normalized JSON; `expected_state.json` is the known-good result of a full build. `check_names.py` stands in for pyflakes (undefined names and unused imports), and CI runs it. `rebuild.sh` builds an instance from nothing and checks it against that golden file; `wait_for_structr.sh` is its readiness check; `tunnel.sh` starts/stops a Cloudflare Tunnel exposing this instance publicly, manually, per session (see `CLOUD.md`).
- `scripts/` — numbered in build order. Most build schema or seed data and verify themselves; `11c` is the selector; `20a`–`22a`, `25a` and `26a` (the planner) are self-cleaning demonstrations of specific fixes against real Structr; `23a` is an offline test; `27` builds two minimal public-facing Structr pages (`/structr/html/home`, `/structr/html/recipes`) via the DOM/Content REST mechanics in `docs/structr-cheatsheet.md` Sec 6. Every script connects through `mealplanner/connection.py`.
- `cloud/` — `setup.sh` (a Claude cloud environment's setup script) and `bootstrap.sh` (brings the stack up in any fresh environment). `CLOUD.md` explains working in a Claude Code cloud session, and the root `CLAUDE.md` holds the working agreements for any session.
...
`bash tools/rebuild.sh` is that loop as a script: it waits for Structr, stops at the first failure, and finishes with the snapshot check below. `bash cloud/bootstrap.sh` does the `.env` and `docker compose up` steps too, generating throwaway passwords if you have none.
...
The planner's optional CP-SAT solver needs OR-tools, which is not part of the base install. To use it, install it into a virtualenv (the tests that need it are skipped without it, and CI runs the suite both without and with it):
