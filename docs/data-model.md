...
| J2 | **StockPolicy resolution.** The governing policy for a Type is found by walking up its hierarchy; the **nearest level with an applicable policy wins**, the same rule `resolveDefault` uses (§8). A policy on an ancestor applies to a descendant **only if `includes_subtypes` is not false**; a policy on the Type itself always applies. Several policies on one Type combine **restrictively**: flags AND together, `eligible_storage_conditions` intersect, `includes_subtypes` is false if any says false, and the largest target level is used. An unset `includes_subtypes` counts as true | §7 says a policy `applies_to` a Type and has `includes_subtypes`, and nothing about which policy governs a Type when several could, or what the flag does when it is false. The implementation had been taking the first policy the graph returned. Restrictive combination is order-independent and can only shrink what counts as usable stock. **Nested policies: the nearest policy owns the physical stock.** A pool leaves out the stock of any pool nested below it, so the same flour cannot satisfy both "keep 2 kg of flour" and "keep 1 kg of bread flour" (`net_requirements`). **Open:** tied policies are treated as constraints on one pool, so "keep 1000 g sealed in the fridge *and* 500 g in the freezer" is not expressible (`resolve_stock_policy`) |
...
| J7 | **Reservation and availability across the Type tree.** A reservation is recorded against one Type, and stock at a Type is everything at or below it, so a reservation reduces stock at every Type above the one it names. A new demand at a Type fits only if, at that Type and at each ancestor, the stock in that subtree minus everything already claimed in that subtree covers it; the Types form a tree, so those are the only constraints, and availability is the smallest such slack. The demanding Type's own StockPolicy filters are applied at every level (an approximation: nested policies can differ) | Reservations were recorded at the ingredient's own Type while stock was counted over its subtree, so a generic "Poultry" demand did not see chicken already reserved, and a chicken demand did not see a generic claim (`reservation.available_for_planning`) |
| J8 | **Inventory is a function of `now`, in grams, and an unusable quantity is an error.** The baseline is the latest observed-or-imputed Measurement at or before `now`; only Processes that began between it and `now` consume. Every quantity is converted to grams (invariant 13) before arithmetic; a missing or unrecognised unit, an input Allocation with no quantity, or a Process with no start time raises rather than counting as zero. A Measurement needs a time (invariant 2) and, with a numeric value, a unit; no QuantitySpecification value or bound may be negative (invariant 12, previously checked on Measurement only); a StockPolicy's target level and reorder threshold must be in the same unit. **Same-instant events, decided:** a Process at the exact instant of the baseline Measurement counts as consuming, so a weighing at the same instant as a use is read as taken *before* it. Nothing recorded says which came first; overstating stock (planning a meal around food already used) is the costlier error. §4.1.1 says "after", so this deviates from it on purpose, and a strict reading would need the model to carry the order, which it does not. **Invariant 15 (no more used than there was) is an audit over these same quantities**, checked at every Process's own time so a later weighing cannot hide an earlier overdraw (`overdraws`, `find_overdraws`); a violation against an `imputed` baseline is informative, as §11 says. It is not a write-time check, since a validator cannot compute on-hand in StructrScript, so an oversized Allocation is accepted when written and found afterwards | The engine ignored `now` when choosing the baseline (a Measurement dated tomorrow was today's stock), subtracted kilograms from grams as bare numbers, and turned bad data into zero. Expiry is likewise bounded by `now`; what it counts from is J12 (`inventory.current_magnitude`) |
| J9 | **Default resolution by (`hasKind`, `keyedBy`).** Start at the Type and walk up; the nearest Type with any *applicable* default wins (a more specific Type beats a more specific key); a default is applicable when all its keys are among the requested keys; the one with the most keys wins, so exact beats partial beats unkeyed; equal specificity is ambiguity and an error | §8 step 5 says each default "resolves independently by its (`hasKind`, `keyedBy`) signature", but the implementation filtered by kind only (`defaults.resolve_default`; see the build sketch's §5 addendum) |
| J10 | **What an exclusion looks at.** For an ExclusionConstraint the Specifications that count are non-optional inputs and outputs. Instruments never count (equipment is not eaten); an optional input does not disqualify, but a recipe that lists an excluded optional ingredient is flagged so it can be left out; an excluded output does (it is what is made). A **soft** ExclusionConstraint is a preference against, not a ban: a candidate that needs a banned Type loses the constraint's weight | Every Specification's Type was checked whatever its role, rejecting recipes for an excluded instrument or optional ingredient, and only `hard` was ever read, so a soft exclusion did nothing (`11c_simple_selector.candidate_consumed_types`, `soft_exclusions`) |
| J11 | **A Plan's expected output is material flowed through its Steps.** A Step's flow is the sum over its inputs of (amount × that input's yield under that Step's transformation). An input's amount is its stated quantity if it has one, else the flow of the Step that produces its Type (an intermediate), else nothing. The Plan's output is the summed flow of its *terminal* Steps, those whose outputs no other Step consumes. A stated amount for an intermediate wins over the computed flow. Where the flow cannot be traced without guessing (an intermediate consumed by several Steps with no stated shares, a Step producing both an intermediate and a final output, a cycle) it is an **error**, not a guess | §5.1.1 applied "each input's own yield, then sum" to one Step. A chained Plan (boil pasta, then toss the boiled pasta with butter) counted the pasta once as a raw input and again as an input of the next Step: 400 g where the hand-worked answer is 200 g. Like J5 it assumes a Type identifies the material (`material_accounting.total_output_grams`) |
| J12 | **Shelf life runs from when the food began to exist.** Expiry is the start of the Process the portion `begins_to_exist_during` (a purchase, or the cook that made a leftover; §5.5) plus its ShelfLife default; where no such Process is recorded, the **earliest** observed-or-imputed mass Measurement made by `now`. Reweighing never restarts it. A recorded origin Process with no start time is an error. The default applied is the one for the container's *current* opened status and storage condition, counted from the origin, so an opened item is judged against its opened shelf life since it began to exist (the conservative reading) | Expiry was the latest weighing plus the shelf life, so weighing food again restarted its clock. **Open:** a portion divided off a larger one begins to exist at the division, and the model has no parent link to trace it back to the purchase, so recording the division as its origin restarts the clock there. The time of *opening* is not modeled either: opened status selects which default applies, not where the clock starts (`inventory.origin_time`, `instance_expiration`) |
| J13 | **Reservation scaling needs a stated yield.** An entry claims its recipe's ingredients scaled by planned servings ÷ recipe yield (§7). A recipe whose yield is not a positive number of servings cannot be scaled, and is an error naming the entry, never "one serving" | The scale defaulted to 1 for an unset yield, so a recipe written for four and planned for two reserved twice what it should, and a zero yield was silently skipped (`reservation.committed_requirements`) |
| J14 | **Variety is the distance to the nearest use.** The score is the number of days between the slot being chosen (or now, if none is given) and the nearest non-skipped entry for the same recipe, past **or planned**, in **any** MealPlan, over a 14-day cap | It measured only "days since", so a recipe planned for next week read as just used, and a skipped entry, which never happened, counted as a use. History is global on purpose: variety is about what the eater has had, not which plan recorded it (`11c_simple_selector.variety_score`) |
| J15 | **A hard `NutritionTarget` is never decided on data that is unknown, placeholder or unmarked.** `NutrientProfile` carries `provenance` (`placeholder` or `sourced`). A recipe's per-serving figure counts as *trusted* only if every profile behind it is `sourced`; a recipe whose figure for a hard-targeted nutrient is unknown or untrusted is ineligible for a plan and is listed as left out. A soft target still uses whatever figure exists | Placeholder nutrient data could satisfy a hard constraint, and the only sign it was placeholder was a substring of the profile's name (REVIEW.md round 1 #24). **Nothing is marked `sourced`:** only a person who has checked a figure can say so, so today every profile is `placeholder` and a hard target leaves recipes out until someone sources their figures (`nutrition_scope.serving_nutrient`, `scripts/12c`) |
| J16 | **A week is chosen as a whole.** Each hard target at its declared scope (`per_meal`, `daily`, `weekly`) is a constraint on the group's final total, and a scoped soft target is scored once on that total, not once per pick against a running total. A slot is a fresh cook or the leftovers of an earlier cook of the same recipe, which must have finished and still be within its keeping time (the ShelfLife default of the `Cooked Leftover` class, Fridge and Sealed: **3 days, a placeholder**). Cooks scale uniformly, so servings cooked = servings eaten at the cook plus at its leftover slots. A week with no feasible plan is reported with the closest plan and its exact shortfall, never returned as a plan | The selector chooses one meal for one slot, so a hard minimum was checked only after the fact. Slots, weights and the weighted sum are proposals, not derivations; the unbuilt parts and measured limits are in `optimizer-design.md` §12 (`mealplanner/planning/`) |
