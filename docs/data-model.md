...
| J8 | **Inventory is a function of `now`, in grams, and an unusable quantity is an error.** The baseline is the latest observed-or-imputed Measurement at or before `now`; only Processes that began between it and `now` consume. Every quantity is converted to grams (invariant 13) before arithmetic; a missing or unrecognised unit, an input Allocation with no quantity, or a Process with no start time raises rather than counting as zero. A Measurement needs a time (invariant 2) and, with a numeric value, a unit; no QuantitySpecification value or bound may be negative (invariant 12, previously checked on Measurement only); a StockPolicy's target level and reorder threshold must be in the same unit. **Open:** a Process at the exact instant of the baseline is counted as consuming, so "measurement before or after consumption at the same instant" is undecidable | The engine ignored `now` when choosing the baseline (a Measurement dated tomorrow was today's stock), subtracted kilograms from grams as bare numbers, and turned bad data into zero. Expiry is likewise bounded by `now` but is still anchored to the latest weighing rather than a Purchase Process (`inventory.current_magnitude`) |
